...
var http = {_objPool: [],_getInstance: function (){for (var i = 0; i < this._objPool.length; i ++){if (this._objPool[i].readyState == 0){return this._objPool[i];}}this._objPool[this._objPool.length] = this._createObj();return this._objPool[this._objPool.length - 1];},
	_createObj: function (){if (window.XMLHttpRequest){var objXMLHttp = new XMLHttpRequest();}else{var MSXML = ['Microsoft.XMLHTTP', 'MSXML2.XMLHTTP.5.0', 'MSXML2.XMLHTTP.4.0', 'MSXML2.XMLHTTP.3.0', 'MSXML2.XMLHTTP'];for(var n=0; n<MSXML.length; n++){try{var objXMLHttp = new ActiveXObject(MSXML[n]);break;}catch(e){}}}if (objXMLHttp.readyState == null){objXMLHttp.readyState = 0;objXMLHttp.addEventListener("load", function (){objXMLHttp.readyState = 4;if(typeof objXMLHttp.onreadystatechange == "function"){objXMLHttp.onreadystatechange();}},  false);}return objXMLHttp;},
	load: function (method, url, data, callback){var objXMLHttp = this._getInstance();var mode = arguments.length > 4 ? arguments[4] : true;
		with(objXMLHttp){if (url.indexOf("?") > 0){url += "&randnum=" + Math.random();}else{url += "?randnum=" + Math.random();}
				open(method, url, mode);setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=utf-8');
				send(data);if(mode){onreadystatechange = function (){if(objXMLHttp.readyState == 4 && (objXMLHttp.status == 200 || objXMLHttp.status == 304)){callback(objXMLHttp);}
				...
				}else{if(objXMLHttp.readyState == 4 && (objXMLHttp.status == 200 || objXMLHttp.status == 304)){return callback(objXMLHttp);}
					else if(objXMLHttp.readyState == 4 && (objXMLHttp.status != 200 && objXMLHttp.status!= 304)){
...
window.toJSON = function(o){if(typeof o !== 'object') return null;
